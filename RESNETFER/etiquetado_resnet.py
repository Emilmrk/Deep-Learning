...
model = load_model('fer2013_resnet50_finetuned_extra.h5')
...
df.to_csv('resultados_etiquetado_resnet50.csv', index=False)

conteo = df['clase'].value_counts().sort_index()

plt.figure(figsize=(8, 6))
conteo.plot(kind='bar', color='skyblue')
plt.title('Conteo de Emociones Etiquetadas (ResNet50)')
plt.xlabel('Emoción')
plt.ylabel('Cantidad')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('grafico_conteo_etiquetado_resnet50.png')
plt.close()
