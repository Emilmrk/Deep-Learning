...
model = tf.keras.models.load_model("fer2013_cnn_baseline_model.h5")
...
for img_name in tqdm(imagenes_crudo, desc="Etiquetando imágenes"):
...
df_resultados.to_csv(os.path.join(carpeta_salida, "clasificaciones_dataset_crudo.csv"), index=False)
...
df_conteo.to_csv(os.path.join(carpeta_salida, "conteo_emociones.csv"), index=False)
...
plt.xlabel("Emocion")
plt.tight_layout()
plt.savefig(os.path.join(carpeta_salida, "grafico_conteo_emociones.png"))
plt.close()
